...
class PreProcessor(object):
    """
        ...
        Constants:
            ...
            PPROCESSED_OUTPUT_FILENAME: The filenamed used for the fully pre-processed sequence file.
    ...
    def preprocess(self):
        ...
        SeqIO.write(unique_seq_records, path.join(self.JOB_LOCAL_PATH,
                                                  self.PPROCESSED_OUTPUT_FILENAME), 'fasta')
    ...
    def _read_tag_seq(cls, seq_path, tag):
        ...
        with open(seq_path, 'r') as f:
            for record in SeqIO.parse(f, 'fasta', generic_protein):
                record.id += f'|{tag}'
                # Weird bug in biopython where if description is not blank it gets
                # appended to the id causing alignment to fail later.
                record.description = ''
                yield record
